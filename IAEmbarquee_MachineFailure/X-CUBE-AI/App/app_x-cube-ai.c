...
#if !defined(AI_MACHINEFAILURE_INPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_in_1[AI_MACHINEFAILURE_IN_1_SIZE_BYTES];
ai_i8* data_ins[AI_MACHINEFAILURE_IN_NUM] = {
data_in_1
};
#else
ai_i8* data_ins[AI_MACHINEFAILURE_IN_NUM] = {
...
#if !defined(AI_MACHINEFAILURE_OUTPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_out_1[AI_MACHINEFAILURE_OUT_1_SIZE_BYTES];
ai_i8* data_outs[AI_MACHINEFAILURE_OUT_NUM] = {
data_out_1
};
#else
ai_i8* data_outs[AI_MACHINEFAILURE_OUT_NUM] = {
...
AI_ALIGNED(32)
static uint8_t pool0[AI_MACHINEFAILURE_DATA_ACTIVATION_1_SIZE];

ai_handle data_activations0[] = {pool0};

/* AI objects ----------------------------------------------------------------*/

static ai_handle machinefailure = AI_HANDLE_NULL;
...
static int ai_boostrap(ai_handle *act_addr)
{
  ...
  err = ai_machinefailure_create_and_init(&machinefailure, act_addr, NULL);
  if (err.type != AI_ERROR_NONE) {
    ai_log_err(err, "ai_machinefailure_create_and_init");
    return -1;
  }

  ai_input = ai_machinefailure_inputs_get(machinefailure, NULL);
  ai_output = ai_machinefailure_outputs_get(machinefailure, NULL);

#if defined(AI_MACHINEFAILURE_INPUTS_IN_ACTIVATIONS)
  /*  In the case where "--allocate-inputs" option is used, memory buffer can be
   *  used from the activations buffer. This is not mandatory.
   */
  for (int idx=0; idx < AI_MACHINEFAILURE_IN_NUM; idx++) {
   ...
   */
  for (int idx=0; idx < AI_MACHINEFAILURE_OUT_NUM; idx++) {
...
static int ai_run(void)
{
  ...
  batch = ai_machinefailure_run(machinefailure, ai_input, ai_output);
  if (batch != 1) {
    ai_log_err(ai_machinefailure_get_error(machinefailure),
        "ai_machinefailure_run");
    return -1;
  }
